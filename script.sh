...
if [ ${yn,,} = "y" ]; then
	...
	grep access_ip system.log | \
		sed -e "s/.*://" | \
		xargs -I {} \
		firewall-cmd --direct --add-rule ipv4 filter DOCKER-USER 1 ! -s {} -p tcp --dport 80 -j DROP
	grep access_ip system.log | \
		sed -e "s/.*://" | \
		xargs -I {} \
		firewall-cmd --direct --add-rule ipv4 filter DOCKER-USER 1 ! -s {} -p tcp --dport 443 -j DROP
	grep access_ip system.log | \
		sed -e "s/.*://" | \
		xargs -I {} \
		echo "firewall-cmd --direct --remove-rule ipv4 filter DOCKER-USER 1 ! -s {} -p tcp --dport 80 -j DROP">>remove_firewall.sh && \
	grep access_ip system.log | \
		sed -e "s/.*://" | \
		xargs -I {} \
		echo "firewall-cmd --direct --remove-rule ipv4 filter DOCKER-USER 1 ! -s {} -p tcp --dport 443 -j DROP">>remove_firewall.sh && \
